...
contract Vault {
    ...
    // use mapping to store the permissions
    mapping(address => mapping (address => uint)) public can;
    // give the permission to usr to act on behalf of msg.sender
    function hope(address usr) external { can[msg.sender][usr] = 1; }

    // remove the permission to usr to act on behalf of msg.sender
    function nope(address usr) external { can[msg.sender][usr] = 0; }

    // check if the usr has the permission to act on behalf of bit
